...
The system is split into a TypeScript frontend (Next.js on Vercel) and a Python backend (FastAPI on Hugging Face Spaces) rather than a single full-stack TypeScript application.

The primary motivation is portfolio positioning: the target audience is AI/ML engineering roles at industrial technology companies, where Python proficiency and understanding of ML infrastructure are table stakes. A Python backend lets the portfolio showcase document parsing (Docling), local embedding models (nomic-embed-text via FastEmbed), reranking (FlashRank), and vector search orchestration — all of which are invisible in a TypeScript-only stack.
