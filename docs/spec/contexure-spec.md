...
- **Backend / RAG Engine Layer**: FastAPI (Python 3.11+) running as a native Web Service on Render.com with build-time model pre-warming and startup lifespan caching. Hosts the core ingestion pipeline, local embedding runtime, reranking engine, and retrieval orchestration.
...
- **Document Parsing**: Utilizes Docling (IBM Research) / PyMuPDF in the Python backend. Docling executes layout analysis to extract markdown representations preserving heading hierarchies (`#`, `##`, `###`), list structures, and complex tabular data layouts.
...
- **Embedding Model**: `BAAI/bge-small-en-v1.5` loaded locally via `fastembed` (ONNX Runtime) on the Python backend. Uses 384-dimensional embeddings with low memory consumption (<40MB RAM). Embeddings are generated with the appropriate query prefix (`search_query: `) or document prefix (`search_document: `).

### Retrieval, Reranking & Guardrail Pipeline

- **Vector Retrieval**: Queries are converted to 384-dim embeddings and sent to Qdrant Cloud to fetch top-20 candidate child chunks using cosine distance.
